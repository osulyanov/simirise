...
module ActiveAdminAddons
  class MyTagBuilder < CustomBuilder
    def render
          ...
          context.select do
            possible_values.each do |val|
              context.option(value: val, selected: val == data) do
                context.text_node I18n.t("activerecord.attributes.#{model.model_name.i18n_key}.#{attribute.to_s.pluralize}.#{val}")
              end
            end
          end
        end
    ...
    def display_data
      @enum_attr == :enumerize ? data.text : I18n.t("activerecord.attributes.#{model.model_name.i18n_key}.#{attribute.to_s.pluralize}.#{data}")
    end
...
ActiveAdminAddons::MyTagBuilder.create_view_methods
