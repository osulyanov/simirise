...
class Lola
  ...
  def initialize(user, payload)
    @user = user
    Rails.logger.info " > Lola USER=#{user.inspect}"
    @payload = payload
  ...
  def get_started
    payload_jsons.each do |payload_json|
      Rails.logger.info " > payload_json=#{payload_jsons}"
  ...
  def call_custom_action
    payload_jsons.each do |payload_json|
      Rails.logger.info " > payload_jsons=#{payload_json}"
  ...
  def process_payload(orig_payload, obj_id)
    if new_payload = orig_payload.dig(:template)&.clone
      ...
      Rails.logger.info " > new_payload=#{new_payload.inspect}"
